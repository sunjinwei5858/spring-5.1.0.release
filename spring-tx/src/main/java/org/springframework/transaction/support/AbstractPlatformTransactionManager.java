...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
    ...
    public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
        ...
        // 如果当前线程存在事务，那么转向嵌套事务的处理
        if (isExistingTransaction(transaction)) {
            // Existing transaction found -> check propagation behavior to find out how to behave.
            /**
             * 因为spring支持多种事务的传播规则，比如PROPAGATION_MANDATORY,PROPAGATION_REQUIRES_NEW等
             * 这些都是在已经存在事务的基础上进行一步的处理，那么对于已经存在的事务，准备操作如何进行？
             */
    ...
    /**
     * 处理已经存在事务：
     * PROPAGATION_REQUIRES_NEW：表示当前方法必须在它自己的事务里运行，一个新的事务将被启动，而如果有有一个事务正在运行，则在这个方法运行期间将被刮起
     *  suspend将原事务挂起，目的是为了在当前事务执行完毕之后在将原来事务还原。
     * PROPAGATION_NESTED：嵌套事务，如果有一个事务正在运行中，则该方法应该运行在一个嵌套的事务中，被嵌套的事务可以独立于封装事务进行提交或者回滚
     *
