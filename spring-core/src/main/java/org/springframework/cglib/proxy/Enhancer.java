...
/**
 * spring自定义了一个Enhancer，关键就是继承AbstractClassGenerator抽象父类，抽象父类实现了ClassGenerator这个接口
 * 这个接口属于ASM框架的接口，ASM属于字节码增强技术的框架，属于指令层次上操作字节码。
 * cglib就是使用ASM技术进行创建代理的
 * 用到的设计模式：访问者模式，
 * 核心API：
 * ClassReader 用于读取已经编译好的.class文件
 * ClassWriter 用于重新构建编译好的类，比如修改类名/属性/方法，也可以生成新的字节码文件
 * 各种Visitor  访问者模式 对于字节码文件中不同的区域有不同的visitor
 * 比如访问方法的MethodVistor aop的实现就是使用这个visotor
 * 访问类变量的FieldVistor
 * 访问注解的AnnotationVisitor
 * <p>
 * <p>
