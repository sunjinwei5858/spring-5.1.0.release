...
/**
 * spring使用该工具类：
 * 1。isFactoryDereference：判断name有没有加&
 * 2. beanNamesForTypeIncludingAncestors：根据传入的class type 查找name[] 数组
 * 3. transformedBeanName：格式化 比如 将&去掉；将别名去掉
 *
 *
...
public abstract class BeanFactoryUtils {
    ...
    /**
     * 对于FactoryBean 判断name有没有加&,
     * 如果加了& 返回true 最终返回的bean是FactoryBean的实现类；
     * 如果没加& 返回false 最终返回的bean是getObject方法的bean
     * <p>
    ...
    /**
     * 事务：type为Advisor 不需要进行缓存 BeanFactoryAdvisorRetrievalHelper#findAdvisorBeans()
     * aop：type为Object 消耗性能 所以会进行缓存 BeanFactoryAspectJAdvisorsBuilder#buildAspectJAdvisors()
     * <p>
     * Get all bean names for the given type, including those defined in ancestor
     * factories. Will return unique names in case of overridden bean definitions.
