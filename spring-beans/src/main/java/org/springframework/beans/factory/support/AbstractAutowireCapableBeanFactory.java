...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	public Object applyBeanPostProcessorsAfterInitialization(Object existingBean, String beanName)
		...
		for (BeanPostProcessor processor : beanPostProcessors) {
			 ...
			 * 我们aop @EnableAspectJAutoProxy --> AnnotationAwareAspectJAutoProxyCreator
			 * 我们事务注解 @EnableTransactionManagement --> InfrastructureAdvisorAutoProxy
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		/**
		 * 这里生成单例对象的ObjectFactory，为添加到三级缓存做准备！！！ wrappedObject
		 */
		 ...
		 * 1 如果该bean是单例，
		 * 2 并且allowCircularReferences=true 表示允许循环依赖的出现
		 * 3 该bean正在创建
		 * 有以上三点 那么标识该bean可以在实例化的时候提前暴露原始对象引用
		 */
		...
		if (earlySingletonExposure) {
			...
			ObjectFactory objectFactory = new ObjectFactory() {
				...
				public Object getObject() {
					...
					// 非常重要
					// 普通bean：在不存在aop代理的情况下 该方法直接返回 仅仅进行了实例化操作的对象 属性还未被赋值 该工厂用于提前曝光对象实例
					// 代理bean：一般是aop smart的beanPostProcessor后置处理器会进行替换成代理对象 这样循环依赖的时候 普通bean注入的属性就是代理bean
	...
	protected Object getEarlyBeanReference(String beanName, RootBeanDefinition mbd, Object bean) {
		...
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				 ...
				 *
				 * ps：在不存在aop代理的情况下，该方法直接返回原始bean对象
				 *
	...
	/**
	 * 在这里进行判断是不是InstantiationAwareBeanPostProcessor后置处理器
	 * 如果是 那么进行调用postProcessBeforeInstantiation()方法
	 * <p>
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				...
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
					/**
					 * postProcessAfterInstantiation如果返回false 那么就不能进行属性注入了
					 */
		...
		/**
		 * 如果是false 那么直接return 不进行populate 属性注入。
		 * 之前没有研究InstantiationAwareBeanPostProcessor这个后置处理器，所以肯定看不懂这段代码
		 */
		...
		if (hasInstAwareBpps) {
			...
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				/**
				 * 在此处获取到AutowiredAnnotationBeanPostProcessor处理器进行@Autowired属性注入
				 */
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
					/**
	...
	/**
	 * 实现父类FactoryBeanRegistrySupport的postProcessObjectFromFactoryBean，针对FactoryBean的后置处理器回调方法
	 * 【其实就是回调bean初始化后的后置处理】
	 * <p>
