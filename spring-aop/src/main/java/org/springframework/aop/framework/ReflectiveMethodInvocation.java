...
public class ReflectiveMethodInvocation implements ProxyMethodInvocation, Cloneable {
    ...
    /**
     * 设计巧妙的地方 责任链模式
     * jdk和cglib动态代理执行通知的方法
     * 而cglib中的内部类继承了该类 重写了proceed方法
     *
     * @return
     * @throws Throwable
     */
    ...
    public Object proceed() throws Throwable {
        ...
        /**
         * 声明了一个currentInterceptorIndex变量，来进行递归调用，这里是递归终止的条件!!!即执行目标方法
         */
        if (this.currentInterceptorIndex == this.interceptorsAndDynamicMethodMatchers.size() - 1) {
            /**
             * invokeJoinpoint 这个就是目标方法
             */
            return invokeJoinpoint();
        }
        /**
         * currentInterceptorIndex进行前++判断
         */
        ...
        if (interceptorOrInterceptionAdvice instanceof InterceptorAndDynamicMethodMatcher) {
            ...
            /**
             * 这里是进行传入递归调用找通知，根据interceptorsAndDynamicMethodMatchers集合的顺序 不断的往下找
             * length为5，
             * index = 0：exposeInvacationInterceptor
             * index = 1：AfterThrowing
             * index = 2：AfterReturning
             * index = 3：After
             * index = 4：Before
             * 无异常情况：before方法--目标方法target--后置通知after--返回通知returning
             * 有异常情况：before方法--目标方法target--后置通知after--异常异常throwing
             */
