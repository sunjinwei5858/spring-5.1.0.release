...
public class RootBeanDefinition extends AbstractBeanDefinition {
    ...
    /**
     * 原来这个postProcessed属性默认为false，看英文注释的意思：这个的作用是为了MergedBeanDefinitionPostProcessor后置处理器可以扩展
     * 如果自己不想MergedBeanDefinitionPostProcessor后置处理器扩展，那么可以将这个设置true 进行关闭.
     * MergedBeanDefinitionPostProcessor：这个后置处理器目前spring的触发时机是在bean已经构造实例化调用,加入三级缓存早期对象引用之前
     *
